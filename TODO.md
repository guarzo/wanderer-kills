...
**Changes Made:** Added `@http_client` module attribute and simplified `http_client/0` function to use the attribute.
...
**Complexity:** Low - straightforward refactor
**Changes Made:** Changed `HttpClient.get_with_rate_limit(url)` to `http_client().get_with_rate_limit(url)`.
...
### 14. Test Helper Alias Consolidation
...
### 18. Test Exception Handling Improvement
...
### 19. Compile Error Fix
**File:** `lib/wanderer_kills/core/systems/killmail_processor.ex:129-146`
**Issue:** The catch clause is used outside of a try block, which causes a compile error.
