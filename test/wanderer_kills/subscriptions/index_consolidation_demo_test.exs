defmodule WandererKills.Subscriptions.IndexConsolidationDemoTest do
  ...
  alias WandererKills.Observability.{CharacterSubscriptionHealthNew, SystemSubscriptionHealthNew}
  
  describe "character index with shared patterns" do
    setup do
      setup_index(CharacterIndexNew, [123_456, 789_012, 345_678])
    end
    
    test "basic operations work identically", %{index_module: index_module, test_entities: test_entities} do
      test_basic_operations(index_module, test_entities)
    end
    
    test "statistics tracking is consistent", %{index_module: index_module, test_entities: test_entities} do
    ...
    test "health integration works", %{index_module: index_module, test_entities: test_entities} do
      test_health_integration(CharacterSubscriptionHealthNew, index_module, test_entities)
    end
  end
  
  describe "system index with shared patterns" do
    setup do
      setup_index(SystemIndexNew, [30_000_142, 30_000_144, 30_000_148])
    end
    ...
    test "health integration works", %{index_module: index_module, test_entities: test_entities} do
      test_health_integration(SystemSubscriptionHealthNew, index_module, test_entities)
    end
  end
  ...
  describe "consolidation verification" do
    test "both indexes implement identical interfaces" do
      ...
      char_exports = CharacterIndexNew.__info__(:functions) |> Enum.sort()
      sys_exports = SystemIndexNew.__info__(:functions) |> Enum.sort()
    ...
    test "both health modules implement identical interfaces" do
      ...
      char_health_exports = CharacterSubscriptionHealthNew.__info__(:functions) |> Enum.sort()
      sys_health_exports = SystemSubscriptionHealthNew.__info__(:functions) |> Enum.sort()
    ...
    test "shared test patterns work for both entity types" do
      ...
      char_entities = [123_456, 789_012]
      sys_entities = [30_000_142, 30_000_144]
      
      # Should work identically
      test_basic_operations(CharacterIndexNew, char_entities)
      test_basic_operations(SystemIndexNew, sys_entities)
