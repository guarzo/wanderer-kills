defmodule WandererKills.Subscriptions.WebhookNotifierTest do
  ...
  describe "notify_webhook/4" do
    test "successfully sends webhook notification" do
      ...
      |> expect(:post, fn url, body, opts ->
        ...
        assert body[:system_id] == 30000142
        assert body[:kills] == kills
        assert opts[:headers] == [{"Content-Type", "application/json"}, {"User-Agent", "WandererKills/1.0"}]
        
        {:ok, %{status: 200, body: %{"success" => true}}}
      end)
      
      assert :ok = WebhookNotifier.notify_webhook(subscription["callback_url"], 30000142, kills, subscription["id"])
    end
    ...
    test "handles webhook failure gracefully" do
      ...
      assert {:error, _} = WebhookNotifier.notify_webhook(subscription["callback_url"], 30000142, kills, subscription["id"])
    end
  ...
  describe "notify_webhook_count/4" do
    test "successfully sends kill count notification" do
      ...
      assert :ok = WebhookNotifier.notify_webhook_count(subscription["callback_url"], 30000142, 42, subscription["id"])
    end
    ...
    test "handles kill count notification failure" do
      ...
      # Should return an error tuple
      assert {:error, _} = WebhookNotifier.notify_webhook_count(subscription["callback_url"], 30000142, 42, subscription["id"])
    end
  end
