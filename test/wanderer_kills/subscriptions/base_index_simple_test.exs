defmodule WandererKills.Subscriptions.BaseIndexSimpleTest do
  ...
  test "BaseIndex shared functions work correctly" do
    ...
    result = WandererKills.Subscriptions.BaseIndex.find_subscriptions_for_entities(
      table_name, [123, 456], :test_entity
    )
    assert length(result) == 2
    assert "sub_1" in result
    assert "sub_2" in result
