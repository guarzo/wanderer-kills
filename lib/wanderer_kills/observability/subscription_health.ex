  ...
  **Subscription Counts:**
    ...
    quote do
      ...
      def check_health(_opts \\ []) do
        WandererKills.Observability.SubscriptionHealth.check_health(
          @index_module, @entity_type, @entity_type_string, @component_name
        )
      end
  ...
  defp check_index_performance(index_module, entity_type_string) do
    try do
      ...
      cond do
        time_microseconds > 10_000 ->  # > 10ms
          %{
            status: :unhealthy,
            message: "#{entity_type_string |> String.capitalize()} lookup took #{time_microseconds}μs (>10ms)",
            duration_us: time_microseconds,
            threshold: "10ms"
          }
          
        time_microseconds > 1_000 ->   # > 1ms
          %{
            status: :degraded,
            message: "#{entity_type_string |> String.capitalize()} lookup took #{time_microseconds}μs (>1ms)",
            duration_us: time_microseconds,
            threshold: "1ms"
          }
  ...
  defp check_subscription_counts(index_module, entity_type_string) do
    try do
      ...
      entity_entries_key = String.to_atom("total_#{entity_type_string}_entries")
  ...
  defp collect_metrics(index_module, entity_type_string) do
    try do
      ...
      # Get entity-specific keys
      entity_entries_key = String.to_atom("total_#{entity_type_string}_entries")
      entity_subscriptions_key = String.to_atom("total_#{entity_type_string}_subscriptions")
      ...
      %{
        ...
        avg_entities_per_subscription: calculate_avg_entities_per_subscription(stats, entity_subscriptions_key),
  ...
  defp measure_response_time(index_module) do
    ...
    Float.round(time_microseconds / 1000, 2)  # Convert to milliseconds
  end
  
  defp get_test_entity_id(entity_type_string) do
    case entity_type_string do
      "character" -> 123_456  # Typical character ID
      "system" -> 30_000_142  # Jita system ID
      _ -> 1  # Generic test ID
    end
  end
end