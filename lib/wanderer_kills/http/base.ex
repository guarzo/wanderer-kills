defmodule WandererKills.Http.Base do
  ...
  def map_status_code(status) when status in @server_error_range, do: {:error, {:server_error, status}}
  def map_status_code(status) when status in @client_error_range, do: {:error, {:client_error, status}}
  ...
  """
  @spec with_telemetry(String.t(), String.t(), keyword(), fun()) :: {:ok, term()} | {:error, term()}
  ...
  def retry_options(service, opts \\ []) do
    ...
    service_options = case service do
      :esi -> [max_retries: 3]
      :zkb -> [max_retries: 5]
      _ -> []
    end
  ...
  def validate_required_fields(response, required_fields) when is_map(response) do
    ...
    case missing_fields do
      [] -> :ok
      fields -> {:error, Error.validation_error(:missing_fields, "Missing required fields", %{fields: fields})}
    end
  end
  
  def validate_required_fields(_, _), do: {:error, Error.validation_error(:invalid_response, "Response must be a map")}
  ...
  def parse_json(_), do: {:error, Error.parsing_error(:invalid_response_type, "Response body must be string, map, or list")}
