defmodule WandererKills.Subscriptions.BaseIndex do
    ...
    quote do
      ...
      def handle_call({:add_subscription, subscription_id, entity_ids}, _from, state) do
        WandererKills.Subscriptions.BaseIndex.handle_add_subscription(
          @table_name, @entity_type, @entity_type_string, subscription_id, entity_ids, state
        )
      end
      ...
      def handle_call({:update_subscription, subscription_id, new_entity_ids}, _from, state) do
        WandererKills.Subscriptions.BaseIndex.handle_update_subscription(
          @table_name, @entity_type, @entity_type_string, subscription_id, new_entity_ids, state
        )
      end
      ...
      def handle_call({:remove_subscription, subscription_id}, _from, state) do
        WandererKills.Subscriptions.BaseIndex.handle_remove_subscription(
          @table_name, @entity_type, @entity_type_string, subscription_id, state
        )
      end
  ...
  """
  def handle_add_subscription(table_name, entity_type, entity_type_string, subscription_id, entity_ids, state) do
    start_time = System.monotonic_time()
    entity_count = length(entity_ids)
  ...
  """
  def handle_update_subscription(table_name, entity_type, _entity_type_string, subscription_id, new_entity_ids, state) do
    start_time = System.monotonic_time()
  ...
  """
  def handle_remove_subscription(table_name, entity_type, _entity_type_string, subscription_id, state) do
    start_time = System.monotonic_time()
  ...
  defp emit_index_telemetry(:character, operation, duration, metadata) do
    WandererKills.Observability.Telemetry.character_index(operation, duration, metadata)
  end
  
  defp emit_index_telemetry(:system, operation, duration, metadata) do
    WandererKills.Observability.Telemetry.system_index(operation, duration, metadata)
  end
  
  # For test entities or other entity types, emit generic telemetry
  defp emit_index_telemetry(entity_type, operation, duration, metadata) when is_atom(entity_type) do
    :telemetry.execute(
