defmodule WandererKills.Storage.KillmailStore do
  ...
  def init_tables! do
    ...
    :ets.new(@system_fetch_timestamps_table, [:set, :named_table, :public, {:read_concurrency, true}])

    tables = [@killmails_table, @system_killmails_table, @system_kill_counts_table, @system_fetch_timestamps_table]

    # Event streaming tables (if enabled)
    if event_streaming_enabled?() do
      :ets.new(@killmail_events_table, [:ordered_set, :named_table, :public, {:read_concurrency, true}])
  ...
  def get_system_fetch_timestamp(system_id) when is_integer(system_id) do
    case :ets.lookup(@system_fetch_timestamps_table, system_id) do
      ...
      [] ->
        {:error, Error.not_found_error("No fetch timestamp found for system", %{system_id: system_id})}
    end
  end
  ...
  def store_killmail(killmail) when is_map(killmail) do
    ...
    if killmail_id do
      ...
      {:error, Error.validation_error("Killmail missing required killmail_id field", %{})}
    end
  end
