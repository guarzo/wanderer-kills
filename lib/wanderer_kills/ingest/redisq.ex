defmodule WandererKills.Ingest.RedisQ do
  ...
  defp do_poll(queue_id) do
    ...
    case result do
      ...
      {:ok, %{body: %{"package" => %{"killID" => id, "zkb" => zkb}}}} ->
        process_killmail_package(id, zkb, queue_id)
  ...
  defp process_killmail_package(id, zkb, queue_id) do
    Logger.debug("[RedisQ] Processing killmail package",
      killmail_id: id,
      queue_id: queue_id,
    ...
    |> case do
      {:ok, :kill_received} ->
        Logger.debug("[RedisQ] Successfully processed killmail", kill_id: id)
        {:ok, :kill_received}

      {:ok, :kill_older} ->
        Logger.debug("[RedisQ] Killmail ID=#{id} is older than cutoff → skipping.")
        {:ok, :kill_older}

      {:ok, :kill_skipped} ->
        Logger.debug("[RedisQ] Killmail ID=#{id} already ingested → skipping.")
        {:ok, :kill_skipped}
  ...
  defp next_schedule({:ok, :kill_received}, _old_backoff) do
    ...
    Logger.debug("[RedisQ] Killmail received → scheduling next poll in #{fast}ms; resetting backoff.")
