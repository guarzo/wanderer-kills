defmodule WandererKills.Parser do
  @moduledoc """
  Main parser module that coordinates parsing functionality.
  ...
  {:ok, enriched} = Parser.parse_full_and_store(full_killmail, partial_killmail, cutoff_time)

  # Parse a partial killmail
  {:ok, enriched} = Parser.parse_partial(partial_killmail, cutoff_time)

  # Handle skipped kills
  {:ok, :kill_skipped} = Parser.parse_partial(old_killmail, cutoff_time)
  ```
  ...
  def parse_partial(%{"killID" => id, "zkb" => %{"hash" => hash}} = partial, cutoff) do
    ...
    case WandererKills.Esi.Cache.get_killmail(id, hash) do
      {:ok, full} ->
