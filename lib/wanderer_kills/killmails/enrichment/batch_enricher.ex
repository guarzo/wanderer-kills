defmodule WandererKills.Killmails.Enrichment.BatchEnricher do
  ...
  defp collect_entity_ids(killmails) do
    Enum.reduce(killmails, %{characters: MapSet.new(), corporations: MapSet.new(), 
                            alliances: MapSet.new(), ships: MapSet.new()}, fn killmail, acc ->
      acc
  ...
  defp fetch_batch(type, ids) when is_list(ids) do
    ...
    {cached, missing} = Enum.split_with(ids, fn id ->
    ...
    fetched_data =
      ...
      |> Flow.reduce(fn -> %{} end, fn
        {id, {:ok, data}}, acc -> Map.put(acc, id, data)
        _, acc -> acc
      end)
      |> Enum.to_list()
      |> hd()
