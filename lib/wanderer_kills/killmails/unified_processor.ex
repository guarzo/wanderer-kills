defmodule WandererKills.Killmails.UnifiedProcessor do
  ...
  def process_batch(killmails, cutoff_time, opts \\ []) when is_list(killmails) do
    ...
    if validate_only? do
      ...
      final_killmails = if enrich? and not Enum.empty?(validated_killmails) do
        {:ok, enriched} = BatchEnricher.enrich_killmails_batch(validated_killmails)
    ...
    case {partial["killmail_id"], partial["zkb"]} do
      {id, zkb} when is_integer(id) and is_map(zkb) ->
        ...
        with {:ok, full_data} <- ESIFetcher.fetch_full_killmail(id, zkb),
             {:ok, merged} <- DataBuilder.merge_killmail_data(full_data, partial) do
    ...
    case extract_system_id(killmail) do
      {:ok, system_id} ->
        Task.start(fn ->
          ...
          Store.put(killmail["killmail_id"], system_id, killmail)
        end)
